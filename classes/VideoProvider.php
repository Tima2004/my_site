<?php
class VideoProvider{
    public static function getNextEpisode($conn, $currentVideo){
        $query = $conn->prepare("SELECT * FROM videos WHERE entityId=:entityId AND id != :videoId AND (season=:season AND episode>:episode) OR season>:season
        ...
        if ($query->rowCount() == 0){
            $query = $conn->prepare("SELECT * FROM videos WHERE season <= 1 AND episode <= 1 AND AND id != :videoId
    ...
    public static function getEntityForUser($conn, $entityId, $username) {
